...
public class CourtDAO {
    ...
    public static List<CourtBean> getAllCourts() {
        ...
        try {
            ...
            String sql = "SELECT * FROM court ORDER BY courtid";
    ...
    public void deleteCourt(int courtID) {
        try {
            con = ConnectionManager.getConnection();
            ps = con.prepareStatement("DELETE FROM court WHERE courtid = ?");
            ps.setInt(1, courtID);
    ...
    public void updateCourt(CourtBean court) {
        ...
        try {
            con = ConnectionManager.getConnection();
            ps = con.prepareStatement("UPDATE court SET court_name = ? WHERE courtid = ?");
            ps.setString(1, CourtName);
            ps.setInt(2, CourtID);
    ...
    public static CourtBean getCourtByID(int CourtID) {
        ...
        try {
            con = ConnectionManager.getConnection();
            ps = con.prepareStatement("SELECT * FROM court WHERE courtid = ?");
            ps.setInt(1, CourtID);
            rs = ps.executeQuery();
